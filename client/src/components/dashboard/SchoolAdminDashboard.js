...
const SchoolAdminDashboard = ({ user }) => {
  ...
  // Add a check for inactive subscription
  const isInactive = usage && usage.plan && !usage.plan.isActive && usage.plan.canActivate !== undefined ? !usage.plan.isActive : false;
    ...
    <div className="space-y-8">
      ...
      {usage && (
        <div className={`rounded-xl shadow-lg border p-8 ${
                ...
                <p className="text-sm text-gray-600">
                  ...
                  {!usage.plan.canActivate && (
                    <span className={`font-medium ${
                      usage.plan.daysRemaining === null || usage.plan.daysRemaining > 14
                        ? 'text-green-700'
                        : usage.plan.daysRemaining > 7
                        ? 'text-yellow-700'
                        : 'text-red-700'
                    }`}>
                      {usage.plan.daysRemaining ? `${usage.plan.daysRemaining} days remaining` : 'Active subscription'}
                    </span>
                  )}
                  {!usage.plan.canActivate && ` • Monitor your plan usage and limits`}
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              ...
              {/* Always show link to subscription/pricing if not active */}
              {!usage.plan.isActive && (
                <Link
                  to="/subscription/pricing"
                  className="px-6 py-3 bg-gradient-to-r from-purple-500 to-indigo-500 text-white rounded-lg hover:from-purple-600 hover:to-indigo-600 font-semibold transition-all shadow-md"
                >
          ...
          {!usage.plan.canActivate && (
            <div className={`rounded-lg p-6 shadow-md border mb-8 transition-all duration-300 ${
              usage.plan.daysRemaining === null || usage.plan.daysRemaining > 14
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">{usage.plan.name}</h3>
                ...
                {usage.plan.daysRemaining !== null && (
                  <div className="mt-4">
                    <div className="flex justify-between text-xs text-gray-600 mb-1">
                      ...
                      <span>{usage.plan.daysRemaining} days left</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full transition-all duration-500 ${
                          usage.plan.daysRemaining > 14
                            ? 'bg-gradient-to-r from-green-500 to-emerald-500'
                            : usage.plan.daysRemaining > 7
                            ? 'bg-gradient-to-r from-yellow-500 to-orange-500'
                            : 'bg-gradient-to-r from-red-500 to-pink-500'
                        ...
                        style={{ 
                          width: `${Math.max(0, Math.min(100, (usage.plan.daysRemaining / 30) * 100))}%` 
                        }}
                      />
                    </div>
                  </div>
      ...
      {/* If subscription is not active, hide all other dashboard features */}
      {usage && usage.plan && usage.plan.isActive && (
        <>
          {/* Quick Actions */}
